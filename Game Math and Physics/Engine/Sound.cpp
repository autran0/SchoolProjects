...
CSoundManager::CSoundManager(): m_nCount(0), g_bMuted(FALSE){
...
int CSoundManager::play(int index){
  ...
  if(g_bMuted)return -1; //bail if muted
...
void CSoundManager::move(Vector3 ePos, int instance, int index){
  ...
  Vector3 v = Vector3(g_cWindow.GetWidth()/2.0f, g_cWindow.GetHeight()/2.0f, 0);
...
void CSoundManager::mute(){
  ...
  if(g_bMuted) //stop all sounds
    for(int i=0; i<m_nCount; i++) //all sounds
      for(int j=0; j<m_nInstanceCount[i]; j++) //all instances
        m_pInstance[i][j]->Stop(false);
} //mute