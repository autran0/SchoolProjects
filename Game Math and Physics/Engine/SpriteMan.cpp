...
/// Reader function for sprite width.
/// \param n Index of sprite.
/// \return Width in pixels of sprite at that index.

int CSpriteManager::GetSpriteWidth(const int n){
  if(m_pSprite[n])
    return m_pSprite[n]->m_nWidth;
  else return 64;
} //GetSpriteWidth

/// Reader function for sprite height.
/// \param n Index of sprite.
/// \return Height in pixels of sprite at that index.

int CSpriteManager::GetSpriteHeight(const int n){
  if(m_pSprite[n])
    return m_pSprite[n]->m_nHeight;
  else return 64;
} //GetSpriteHeight

/// Reader function for sprite width and height.
/// \param w [out] Sprite width in pixels.
/// \param h [out] Sprite height in pixels.
/// \param n Index of sprite.

void CSpriteManager::GetSpriteSize(int& w, int& h, const int n){
  if(m_pSprite[n]){
    w = m_pSprite[n]->m_nWidth; 
    h = m_pSprite[n]->m_nHeight;
  } //if
  else w = h = 64;
} //GetSpriteSize