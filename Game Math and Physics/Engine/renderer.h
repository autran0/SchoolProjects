...
class CRenderer{
  ...
  public:
    ...
    void Initialize(CWindow& wc); ///< Initialize Direct3D.
    ...
    void move(float x, float y); ///< Move the camera.
    ...
    void drawtext(const char* text, const BOOL centered = TRUE); ///< Draw text to screen.
    void GetWorldSize(int &x, int &y); ///< Get the Render World width and height.
    void Release(); ///< Release textures and vertex buffer.

    int GetWidth(const int n); ///< Get sprite width in pixels.
    int GetHeight(const int n); ///< Get sprite height in pixels.
    void GetSize(int& w, int& h, const int n); ///< Get sprite size in pixels.
}; //CRenderer