...
void CRenderer::Initialize(CWindow& wc){ 
  HWND hwnd = wc.GetHwnd();
  wc.GetSize(m_nScreenWidth, m_nScreenHeight);
  ...
  if(FAILED(hr))
    ABORT("Failed to create depth buffer.");

  hr = CreateRasterizer();
  if(FAILED(hr))
    ABORT("Failed to create rasterizer.");

  CreateViewport();
  
  //transformation matrices
  float w = m_nScreenWidth/2.0f;
  float h = m_nScreenHeight/2.0f;

  m_matView = XMMatrixLookAtLH(Vector3(w, h, -350.0f), Vector3(w, h, 1000.0f), Vector3(0, 1, 0));
  m_matProj = XMMatrixOrthographicOffCenterLH(-w, w, -h, h, 1.0f, 1000000.0f);
...
/// This is not needed in these demos, but we might find a use for it.

void CRenderer::move(float x, float y){
  m_matView = XMMatrixLookAtLH(Vector3(x, y, -350.0f), Vector3(x, y, 1000.0f), Vector3(0, 1, 0));
} //move
...
void CRenderer::InitBackground(){
  ...
  //load vertex buffer
  float w = (float)m_nScreenWidth;
  float h = (float)m_nScreenHeight;
...
void CRenderer::DrawBackground(){
  SetWorldMatrix(Vector3(0.0f, 0.0f, 100000.0f));
...
void CRenderer::draw(int sprite, const Vector3& s, float angle, float xsize, float ysize){   
  SetWorldMatrix(s, angle, xsize, ysize);
...
/// \param centered TRUE if text is centered, FALSE if top right of screen

void CRenderer::drawtext(const char* text, const BOOL centered){  
  //center the text on the screen
  Vector3 s;
  
  if(centered)
    s = Vector3((m_nScreenWidth - strlen(text)*m_pScreenText->m_nFrameWidth)/2.0f,
      m_nScreenHeight/2.0f, -1000.0f);
  else
    s = Vector3(1.5f*(float)m_pScreenText->m_nFrameWidth, 
      (float)m_nScreenHeight - m_pScreenText->m_nFrameHeight, -1000.0f);
  ...
  for(unsigned int i=0; i<strlen(text); i++){
    ...
    if(c >='A' && c <= 'Z'){ //upper case alphabetic
      ...
      y = 1;
      switch(c){ 
        case ',':  xoffset = 10; break;
        case '.':  xoffset = 11; break;
        case '+':  xoffset = 12; break;
        case '-':  xoffset = 13; break;
        case '&':  xoffset = 14; break;
        case ':':  xoffset = 15; break;
        case '\'': xoffset = 16; break;
        case ' ':  xoffset = 17; break;
        case '\n': 
          s.x = 0.5f*(float)m_pScreenText->m_nFrameWidth; 
          s.y -= m_pScreenText->m_nFrameHeight; 
          xoffset = 18;
          break;
      } //switch
    } //else
...
HRESULT CRenderer::CreateD3DDeviceAndSwapChain(HWND hwnd){ 
  ...
  d3dscd.BufferCount = 2;
  d3dscd.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM; //color mode
  d3dscd.BufferDesc.Height = m_nScreenHeight; //height
  ...
  d3dscd.BufferDesc.Width = m_nScreenWidth; //width
  ...
  d3dscd.SwapEffect = DXGI_SWAP_EFFECT_FLIP_SEQUENTIAL;
  ...
  hr = pSwapChain->ResizeBuffers(1, m_nScreenWidth, m_nScreenHeight, DXGI_FORMAT_R8G8B8A8_UNORM, 0);
...
HRESULT CRenderer::CreateDepthBuffer(){ 
  ...
  depthStencilDesc.Width = m_nScreenWidth;
  depthStencilDesc.Height = m_nScreenHeight;
...
int CRenderer::GetWidth(const int n){
  return m_cSpriteManager.GetSpriteWidth(n);
} //GetWidth

/// \param n Sprite index.
/// \return Height of sprite in pixels.

int CRenderer::GetHeight(const int n){
  return m_cSpriteManager.GetSpriteHeight(n);
} //GetHeight

/// \param w [OUT] Width of sprite in pixels.
/// \param h [OUT] Height of sprite in pixels.
/// \param n Sprite index.

void CRenderer::GetSize(int& w, int& h, const int n){
  m_cSpriteManager.GetSpriteSize(w, h, n);
} //GetSize